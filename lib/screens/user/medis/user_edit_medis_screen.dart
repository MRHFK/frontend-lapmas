...
class UserEditMedisScreen extends StatefulWidget {
  final dynamic medis;
  final Function onSubmit;

  const UserEditMedisScreen(
      {super.key, required this.medis, required this.onSubmit});

  @override
  _UserEditMedisScreenState createState() => _UserEditMedisScreenState();
}

class _UserEditMedisScreenState extends State<UserEditMedisScreen> {
  ...
  void initState() {
    super.initState();
    _judulLaporan = widget.medis['judul_laporan'];
    _namaPelapor = widget.medis['nama_pelapor'];
    _noTelepon = widget.medis['no_telepon'];
    _lokasiKejadian = widget.medis['lokasi_kejadian'];
    _tanggalKejadian = DateTime.parse(widget.medis['tanggal_kejadian']);
    _deskripsiKejadian = widget.medis['deskripsi_kejadian'];
  }
  ...
  Future<void> _submitLaporan() async {
    ...
    try {
      final response = await http.put(
        Uri.parse('http://localhost:8000/api/medis/${widget.medis['id']}'),
        headers: {
